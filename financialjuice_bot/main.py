#!/usr/bin/env python3
"""
PoladliFX — FinancialJuice RED economic-calendar + RED headlines -> Telegram.

Realtime design:
- One persistent Chromium page is opened against FinancialJuice /home.
- The rendered DOM is inspected every POLL_INTERVAL_SECONDS (default 5 sec).
- ONLY RED / HIGH-IMPACT calendar rows are accepted; orange rows are ignored.
- T-5 pre-alerts are generated from future red calendar events.
- A released red calendar event is sent immediately once its scheduled time is reached.
- 09:00 Baku digest lists today's red calendar events.
- RSS remains an independent backup stream for explicitly marked RED/HIGH-IMPACT headlines.

For true 24/7 realtime behavior, run this process on an always-on host (VPS/container).
GitHub Actions is retained only as a manual/testing runner because its cron scheduler is
not a guaranteed realtime process.
"""
...
PAGE_REFRESH_SECONDS = int(os.getenv("PAGE_REFRESH_SECONDS", "60"))
FRESH_MINUTES = int(os.getenv("FRESH_MINUTES", "12"))
PRE_ALERT_MINUTES = int(os.getenv("PRE_ALERT_MINUTES", "5"))
PRE_ALERT_WINDOW_SECONDS = int(os.getenv("PRE_ALERT_WINDOW_SECONDS", "30"))
...
DAILY_DIGEST_WINDOW_SECONDS = int(os.getenv("DAILY_DIGEST_WINDOW_SECONDS", "90"))
...
def fetch_url(url):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "PoladliFX-FinancialJuice-Bot/4.0",
...
def canonical_id(title, timestamp, source=""):
    ...
    raw = f"{normalized}|{timestamp}|{source}"
...
def was_digest_sent(conn, local_date):
    ...
    return bool(row and row[0] == local_date.isoformat())
...
def format_digest(items, local_date):
    ...
    for item in sorted(items, key=lambda x: x["timestamp"]):
        ...
        title = html.escape(item.get("title") or "Без заголовка")
...
def run_breaking_alerts(conn, items, now_ts):
    ...
    if candidates:
        print(f"Released RED alerts sent: {len(candidates)}")
...
def run_daily_digest(conn, items, now_local):
    ...
    if delta < 0 or delta > DAILY_DIGEST_WINDOW_SECONDS:
        return
    if was_digest_sent(conn, today):
        return

    today_items = [
        x
        for x in items
        if datetime.fromtimestamp(x["timestamp"], timezone.utc).astimezone(BAKU).date() == today
        and x.get("impact") == "red"
    ]
    telegram_send(format_digest(today_items, today))
    mark_digest_sent(conn, today)
    print(f"Daily digest sent: {len(today_items)} RED events")


def extract_red_calendar_events(page):
    """Read the currently rendered FinancialJuice /home DOM and return RED calendar rows."""
    raw = page.evaluate(
        r"""
        () => {
          const out = [];
          ...
          const isRed = el => {
            ...
            for (let i = 0; i < 3 && node; i++, node = node.parentElement) {
              ...
              if (r >= 120 && r > g * 1.6 && r > b * 1.6 && (r - g) >= 70) return true;
            }
            return false;
          };
          const selector = 'a, div, li, tr, article, section';
          for (const el of document.querySelectorAll(selector)) {
            ...
            const timeLine = lines.find(x =>
              /\b\d{1,2}:\d{2}\s+(?:[A-Za-z]{3}|[A-Za-z]+)\s+\d{1,2}(?:\s+\d{4})?\b/.test(x)
            ) || '';
    ...
    for item in raw:
        ...
        m = re.search(
            r"(?P<h>\d{1,2}):(?P<m>\d{2})\s+(?P<mon>[A-Za-z]{3}|[A-Za-z]+)\s+(?P<d>\d{1,2})(?:\s+(?P<y>\d{4}))?",
            time_line,
        )
        if not m:
            continue
    ...
    return sorted(unique.values(), key=lambda x: x["timestamp"])[:MAX_HOME_EVENTS]


def open_financialjuice_page(p):
...
def run_pre_alerts(conn, home_events, now_ts):
    ...
    for event in home_events:
        if event.get("impact") != "red":
            continue
        # Only future events qualify for T-5.
        ...
        message = (
            ...
            f"🔴 FinancialJuice Economic Calendar"
        )
        if event.get("link"):
...
def run_rss_backup(conn, now_ts):
    """Independent RED-only headline backup. Calendar remains a separate source."""
    try:
...
def monitor_forever():
    ...
    with sync_playwright() as p:
        ...
        try:
            browser, context, page = open_financialjuice_page(p)
            last_refresh = time.time()
            print(
                f"Realtime FinancialJuice monitor started: every {POLL_INTERVAL_SECONDS}s; "
                f"page refresh every {PAGE_REFRESH_SECONDS}s; timezone Asia/Baku"
            )

            while True:
                cycle_started = time.time()
                now_ts = int(cycle_started)
                now_local = datetime.fromtimestamp(now_ts, timezone.utc).astimezone(BAKU)

                try:
                    home_events = extract_red_calendar_events(page)
                    print(
                        f"[{now_local.strftime('%Y-%m-%d %H:%M:%S')}] "
                        f"calendar RED rows: {len(home_events)}"
                    )
                    ...
                    print(f"WARNING: FinancialJuice /home DOM read failed: {exc}")

                # RSS is a backup headline stream, polled less often to avoid unnecessary load.
                if cycle_started - last_rss_poll >= 15:
                    run_rss_backup(conn, now_ts)
                    last_rss_poll = cycle_started

                # Refresh the page occasionally so a stale frontend session cannot block updates.
                if cycle_started - last_refresh >= PAGE_REFRESH_SECONDS:
                    try:
                        print("Refreshing FinancialJuice /home page...")
                        ...
                        print(f"WARNING: FinancialJuice page refresh failed: {exc}")
                        try:
                            if context:
                                context.close()
                            if browser:
                            ...
                            print(f"ERROR: FinancialJuice page reopen failed: {reopen_exc}")

                elapsed = time.time() - cycle_started
                time.sleep(max(0.1, POLL_INTERVAL_SECONDS - elapsed))

        except KeyboardInterrupt:
            print("Stopping realtime monitor...")
        finally:
            try:
                if context:
                    context.close()
...
def single_run():
    """One-shot mode for CI/manual tests."""
    conn = db()
    cleanup(conn)
    now_ts = int(time.time())
    now_local = datetime.fromtimestamp(now_ts, timezone.utc).astimezone(BAKU)
    print(f"Single run at {now_local.strftime('%Y-%m-%d %H:%M:%S')} Baku")
...
def main():
    mode = os.getenv("RUN_MODE", "realtime").lower()
    if mode in {"once", "single", "ci"}:
        single_run()
    else:
        monitor_forever()
